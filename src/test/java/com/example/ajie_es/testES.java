...
import com.example.ajie_es.pojo.Law_HTML_txt;
import com.example.ajie_es.pojo.Law_txt;
...
public class testES {
    ...
    @Test
    public void test1(){
        IndexOperations indexOperations = template.indexOps(Law_txt.class);
        indexOperations.create();
        //        定制mapping规则
        Document mapping = indexOperations.createMapping(Law_txt.class);
        indexOperations.putMapping(mapping);

//        IndexOperations indexOperations1 = template.indexOps(Law_HTML_txt.class);
//        indexOperations1.create();
//        Document mapping1 = indexOperations1.createMapping(Law_HTML_txt.class);
//        indexOperations1.putMapping(mapping1);
    }
    ...
    public void test9(){
        ...
        ExecutorService threadPool = new ThreadPoolExecutor(30, 60,
                0L, TimeUnit.MILLISECONDS,
                new ArrayBlockingQueue<Runnable>(50*2));

//        List<Law_txt> list = new ArrayList<>();

        String path = "D:\\BaiduNetdiskDownload\\文书id\\text";		//要遍历的路径
...
//        for(File f:fs) {                    //遍历File[]数组
        ...
        String str = readJsonFile(f);
        //        System.out.println(str);
        JSONObject jsonObject = JSON.parseObject(str);
        Law_txt txt_msg = new Law_txt(jsonObject.getString("s1"),
                jsonObject.getString("s5"),
                jsonObject.getString("s23"),
                jsonObject.getJSONArray("s47"),
                jsonObject.getDate("s41"),
                jsonObject.getDate("s31"),
                jsonObject.getString("s7"),
                jsonObject.getString("s8"),
                jsonObject.getString("s2"),
                jsonObject.getJSONArray("s17"),
                jsonObject.getJSONArray("s45"),
                jsonObject.getJSONArray("s11")
        );
//            Law_HTML_txt html_txt = new Law_HTML_txt(
//                    jsonObject.getString("s5"),
//
//                    jsonObject.getString("qwContent")
//            );


        Law_txt save = template.save(txt_msg);
//            Law_HTML_txt savehtml = template.save(html_txt);
//            System.out.println(save);
